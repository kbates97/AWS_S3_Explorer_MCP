...
@mcp.tool()
def list_s3_objects(bucket_name: str, prefix: str = "", max_keys: int = 50) -> list[dict]:
    """
...
@mcp.tool()
def read_s3_file_head(bucket_name: str, object_key: str, byte_limit: int = 2000) -> str:
    """
    ...
    """
    try:
        # Only fetch a specific byte range to ensure we don't crash the LLM's context window
...
if __name__ == "__main__":
    # Run the MCP server to listen for incoming requests from the host client
    mcp.run()